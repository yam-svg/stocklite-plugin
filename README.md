...
./gradlew buildPlugin
```
输出路径：`build/distributions/stocklite-plugin-1.12.0.zip`
...
### 图表功能
- K 线图底部新增成交量柱显示（红绿配色，与主图涨跌色一致），日内/日K/周K/月K 均支持，悬浮提示同步展示成交量（自动格式化为 万/亿 单位）
- 成交量柱可在设置中开关，默认开启
- 优化图表历史数据拉取范围与数量，减少无谓请求
- 修复期货日K线数据请求长度限制导致的历史数据缺失

### 表格交互
- 所有表格表头排序改为三态循环：升序 → 降序 → 原始顺序，第三下点击还原默认排序
...
### 搜索与数据修复
- 优化搜索框回车事件处理
- 修复按股票代码搜索时部分结果仅显示代码而非股票名称的问题，自动通过行情接口补全真实名称
- 修复港股行情中文名称乱码问题（腾讯 API 编码错误）
