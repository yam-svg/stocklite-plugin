...
### 多窗口数据同步
同时打开多个 IDE 项目窗口时，在任一窗口中添加、编辑或删除股票/基金/期货，其他窗口的面板立即自动更新，无需手动刷新。

### 添加股票时支持查看行情与K线图
搜索并选中股票后，对话框内即时展示当前行情卡片（现价、涨跌幅、昨收）和内嵌走势图，支持切换日内/日K/周K/月K，方便在添加前确认走势；成本价自动预填当前实时价，可手动修改。

### 收益率展示增强
IDE 状态栏持仓弹窗汇总行新增总收益率，持仓明细表格新增"收益率"列，展示每只股票的累计收益率。
