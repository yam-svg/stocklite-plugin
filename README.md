...
部分卖出后，面板盈亏金额和收益率现在包含历史卖出累计的已实现盈亏，不再只显示剩余仓位的浮亏。

**举例：** 持有某股票成本 1000 元，亏损 100 元（-10%），卖出一半后：
- 修复前：持仓盈亏 -50 元，收益率 -10%（只算剩余 5 成仓位的浮亏）
- 修复后：持仓盈亏 -100 元，收益率 -20%（已实现 -50 + 未实现 -50，分母为剩余持仓成本）

同时修复了原始买入记录缺失时（如历史清仓记录早于系统记录）会产生虚假收益的问题。

### 盈亏走势图按历史仓位还原

「盈亏走势」图表不再用当前成本价和持仓数量反推所有历史日期，改为按时间顺序逐笔回放交易记录，还原各历史日期的真实仓位（数量 + 加权均价 + 已实现盈亏）后再计算每日盈亏，使走势曲线与实际持仓变化完全对应。
...
./gradlew buildPlugin
```
输出路径：`build/distributions/stocklite-plugin-1.12.3.zip`
