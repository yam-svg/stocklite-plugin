...
修复全球指数（纳斯达克、韩国综合指数、标普500、道琼斯等）日K/周K/月K 数据不准确的问题。

**根因：** Yahoo Finance v8 API 对 `range=max` 的请求有约 1000 条数据点限制，历史超长的指数超出限制时会静默降频（不报错，直接返回粒度更粗的数据），导致：
- 日K（`interval=1d&range=max`）→ 返回周级数据，与周K完全相同
- 周K（`interval=1wk&range=max`）→ 标普500等历史5000+周的指数被降为月级数据
- 月K（`interval=1mo&range=max`）→ 标普500（自1927年起约1188个月）等被降为季度级数据

**修复：** 改用固定时间窗口（日K: `5y`，周K/月K: `10y`），将数据量控制在安全范围（日K≈1260条，周K≈520条，月K≈120条），消除降频。
...
./gradlew buildPlugin
```
输出路径：`build/distributions/stocklite-plugin-1.12.2.zip`
