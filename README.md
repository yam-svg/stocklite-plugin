...
### 盈亏历史图表
- **单只股票**：在股票行右键 → 交易记录，对话框新增「盈亏走势」Tab，根据全部交易记录重放持仓，结合历史日线收盘价绘制持有期间每日浮动盈亏折线图
- **总持仓**：点击股票面板底部汇总栏文字，弹出总持仓盈亏走势图，并发拉取所有持仓股历史数据后合并显示每日综合盈亏

### 基金 K 线图
点击基金面板的「官方涨跌」或「今日估算」列，面板底部内嵌展开 K 线图（日/周/月），支持所有基金类型：
- 场外基金（纯数字代码）使用天天基金净值历史
- 场内 ETF 使用腾讯前复权 K 线数据

今日估算列现在可在设置 → 基金列设置中关闭。

### K 线图优化
- **A 股前复权**：日K 改用腾讯 qfq 接口，分页拉取全量上市历史，修复 ETF 分红拆分导致的价格断层
- **最高/最低价标注**：滚动或缩放时，可见区间最高/最低价实时标注在对应蜡烛上方/下方
- **中文时间轴**：横坐标和悬浮时间标签改为中文格式（年/月/日）
- **均线默认关闭**：MA5/MA10/MA20 改为默认不显示，在设置 → 功能开关中开启
- **悬浮栏位置优化**：信息栏固定在图表顶部，不再浮动遮挡 K 线
