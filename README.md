...
- **搜索添加**：按股票名称或 6 位代码搜索，输入北交所代码段（920 / 43 / 83 / 87 / 88 开头）自动识别为北交所标的
- **实时行情**：行情自动刷新、涨跌配色、持仓盈亏计算均与沪深 A 股完全一致
- **走势图**：日内分时与日K / 周K / 月K（前复权）正常显示
- **周边功能**：财报日期悬浮提示与临近圆点提醒、右键"在浏览器中打开"（跳转东方财富北交所行情页）均可用

### 盘后多空信号准确率统计

全球面板的盘后多空信号现在会自动留存历史并复盘：

- 每日收盘后自动记录当日预测快照（看多 / 中性 / 看空 + 得分），历史最多保留 60 条
- 次日按当日沪深300实际涨跌结算上一次预测的方向对错
- 多空信号标签后直接显示历史准确率百分比（已结算记录满 5 条起），悬浮可查看近期预测的方向、得分与实际涨跌明细
...
./gradlew buildPlugin
```
输出路径：`build/distributions/stocklite-plugin-1.13.0.zip`
